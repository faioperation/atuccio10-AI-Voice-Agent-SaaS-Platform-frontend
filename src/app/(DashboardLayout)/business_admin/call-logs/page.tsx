...
export default function CallLogsPage() {
  ...
  }, [callLogs, searchTerm, statusFilter, outcomeFilter]);
  ...
  return (
    <div className="w-full">
      <div className="bg-white rounded-2xl shadow-sm border border-[#EDEFF2] min-h-[800px] flex flex-col overflow-hidden">
        ...
        <div className="p-6 pb-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[#F1F4F9]">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-3 py-2 rounded-xl border border-[#EDEFF2] text-[13px] text-[#0C1824] bg-white focus:outline-none focus:ring-2 focus:ring-[#1A6BDC]/20 focus:border-[#1A6BDC] transition-colors hover:cursor-pointer min-w-[130px]"
              >
            ...
            {/* Outcome Filter */}
            <div className="flex items-center gap-2">
              <span className="text-[13px] font-semibold text-[#64748B]">Outcome:</span>
              <select
                value={outcomeFilter}
                onChange={(e) => {
                ...
                className="px-3 py-2 rounded-xl border border-[#EDEFF2] text-[13px] text-[#0C1824] bg-white focus:outline-none focus:ring-2 focus:ring-[#1A6BDC]/20 focus:border-[#1A6BDC] transition-colors hover:cursor-pointer min-w-[155px]"
              >
            ...
            {/* Clear Option */}
            {(statusFilter !== "All" || outcomeFilter !== "All" || searchTerm !== "") && (
              <button
                onClick={() => {
                  setStatusFilter("All");
                ...
                className="text-[12px] font-semibold text-[#1A6BDC] hover:text-[#1558be] hover:underline cursor-pointer transition-colors"
              >
                Clear Filters
              </button>
            )}
          ...
          <table className="w-full text-left border-collapse min-w-[950px]">
            <thead>
              <tr className="border-b border-[#F1F4F9]">
                ...
                <th className="py-4 px-4 text-[12px] font-semibold text-[#64748B]">Outcome</th>
                <th className="py-4 pr-6 pl-4 text-[12px] font-semibold text-[#64748B]">Action</th>
              </tr>
            </thead>
            <tbody>
              {currentLogs.map((log, idx) => (
                <tr key={`${log.id}-${idx}`} className="border-b border-[#F1F4F9] last:border-0 hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <td className="py-[16px] px-4 text-[13px] font-medium text-[#64748B]">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold ${getStatusBadgeClass(log.status)}`}>
                      {log.status}
                    </span>
                  </td>
                  <td className="py-[16px] px-4 text-[13px] font-medium text-[#64748B]">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold ${getOutcomeBadgeClass(log.outcome)}`}>
                      {log.outcome}
                    </span>
                  </td>
              ...
              {currentLogs.length === 0 && (
                <tr>
                  <td colSpan={8} className="text-center py-10 text-[#64748B]">No call logs found matching current search/filters.</td>
                </tr>
              )}
            </tbody>
          </table>
